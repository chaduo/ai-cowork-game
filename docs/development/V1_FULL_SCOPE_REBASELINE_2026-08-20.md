...
- [x] Add provider continuation/rebuild fallback for a resolved blocking decision. — Done 2026-08-17: `ContinuationService` (`backend/app/services/continuation.py`, zhang-owned) resumes the OpenGame provider's SAME session after a blocking decision resolves — NOT a rebuild. `OpenGameAdapter.resume_session` drives `opengame --resume <session_id> -p "<resolved decision>" --yolo ... -o stream-json` against the SAME run/workspace (a paused run's working tree is trusted, so C13's discard-on-cancel/failure does NOT apply). The session id is captured from the run's first `system` event (`_Run.opengame_session_id`), surfaced on `GameBuildResult.metadata["opengame_session_id"]` at `waiting_for_input`, and persisted on a new `Run.opengame_session_id` (migration `0013_opengame_resume`). `resume_session` is a dedicated OpenGame-specific method (the neutral `GameBuildRequest` carries no session id), so `SUPPORTED_OPERATIONS` stays `{"create"}` (modify/repair/test still unsupported). `ContinuationService.resume_build` consumes the resumed events into the same run's sequence and persists the terminal in a single asyncio.run. 6 tests pass (resume→succeeded/failed on same run, events in order, workspace reused, session-not-resumable failure, unresolved-decision guard, owner-boundary invariant); offline full 224/0. API `continue_run` wiring is deferred (app.state.game_agent is FakeGameAgent pre-C19, with no resume surface); the contract + helpers are ready for C19 to wire the real adapter.
