...
- [x] **Step 1: Write failing migration/model tests** for run table, event table, exact sequence uniqueness, and
  idempotent run identity.
- [x] **Step 2: Run the focused tests and verify the new models/table imports fail.**
- [x] **Step 3: Add models and repeatable Alembic migration without changing existing tables.**
- [x] **Step 4: Run migration and focused tests, then the existing backend suite.**
- [x] **Step 5: Commit `feat: persist runs and ordered events`.**
...
- [x] **Step 1: Add failing repository tests** for ordered append/replay, secret redaction, terminal guards,
  cancellation, orphan recovery, and success requiring a confirmed terminal artifact event.
- [x] **Step 2: Run focused tests and verify repository imports/behavior fail.**
- [x] **Step 3: Implement minimal repository transaction methods and bounded diagnostic sanitization.**
- [x] **Step 4: Run focused and full backend tests.**
- [x] **Step 5: Commit `feat: add run event repository`.**
...
- [x] **Step 1: Write failing API tests** for idempotent create, persisted status, JSON replay, SSE replay from
  `Last-Event-ID`, and cancellation without false success.
- [x] **Step 2: Run focused API tests and verify route failures.**
- [x] **Step 3: Implement the thin router and bounded SSE polling generator with terminal/idle close.**
- [x] **Step 4: Run API and full backend tests.**
- [x] **Step 5: Commit `feat: expose run status and event replay`.**
...
- [x] **Step 1: Mark completed plan steps and review for scope leakage.**
- [x] **Step 2: Run `pytest backend/tests -q`, `npm --prefix frontend run build`, and `git diff --check`.**
- [x] **Step 3: Manually curl create/status/replay/cancel and parse SSE IDs from a terminal run.**
- [x] **Step 4: Commit `docs: record run event observability verification`.**
