...
const stage = computed(() => {
  const labels: Record<BuildPhase, string> = {
    ...
    playable_ready: 'First Playable Ready', candidate_ready: 'Build Candidate Ready', build_error: 'Presentation · 需要重试',
  }
...
<template>
  <aside class="cowork-panel build-cowork-panel" aria-label="Cowork AI Build Activity">
    <header class="cowork-panel-header">
      ...
      <span class="cowork-stage" :class="{ ready: phase === 'playable_ready' || phase === 'candidate_ready' }">{{ phase === 'playable_ready' ? 'READY' : phase === 'candidate_ready' ? 'REVIEW' : 'BUILDING' }}</span>
    </header>
