  ...
  if (design.scenarioId === 'coffee-shop') {
    return {
      ...
      characters: {
        ...
        favorRules: '完成顾客任务与日常对话会提升 NPC 好感，好感推动关系阶段变化。',
        relationshipEvents: '达到一定关系后触发新的角色事件。',
        requestRewards: '完成 Emily 或 Alex 的委托会获得好感与额外收入。',
      },
  ...
  if (design.scenarioId !== 'farm') {
    ...
    return {
      ...
      characters: {
        ...
        primaryNpcs: '向导 NPC 会为玩家提供目标。', relationshipGrowth: '完成目标后逐步建立关系。', favorRules: '与 NPC 互动并完成目标会提升好感，好感推动关系阶段变化。', relationshipEvents: '推进目标后触发新的对话。', requestRewards: '完成 NPC 发布的目标会获得好感奖励。',
      },
