  ...
  <article class="build-workspace-view">
    <header class="build-view-title">
      <div>
        ...
        <h1>{{ phase === 'build_error' ? 'Build 未完成' : 'Building your first playable version' }}</h1>
        <p>{{ phase === 'build_error' && errorMessage ? errorMessage : '根据已确认的 GameSpec，逐步完成游戏并验证每一项可玩标准。' }}</p>
      </div>
      <div class="build-view-actions">
        ...
        <span class="working-build-status">
          ...
          {{ phase === 'playable_ready' ? 'PLAYABLE READY' : phase === 'build_error' ? 'BUILD FAILED' : 'WORKING BUILD' }}
        </span>
      </div>
    </header>
    ...
    <div class="build-view-grid">
      ...
      <BuildMilestoneDetail v-else :milestone="activeMilestone" :status="activeStatus" :phase="phase" :error-message="props.errorMessage" @retry="$emit('retry')" />
    </div>
  </article>
</template>
